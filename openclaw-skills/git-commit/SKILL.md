---
name: git-commit
description: Smart git commit with remote sync, amend intelligence, and conventional commits. Use when the user asks to commit changes, stage and commit, save work to git, push code, or sync and commit. Automatically checks remote for new commits before committing and syncs via rebase if needed. Analyzes whether current changes should amend the previous commit. Generates conventional commit messages with agent/model info in the footer.
---
...
Use this skill when:
- User asks to "commit changes", "save work to git", or "stage and commit"
- User mentions pushing code or syncing with remote
- User wants to save progress to version control
- Any situation involving git commit operations

## Prerequisites

- Git repository initialized
- User has write permissions to the repository
- For push operations: remote repository configured and accessible
- User's git author info (check `USER.md` or ask if needed: `Hugo Gu <hugogu@outlook.com>`)
...
Always append agent and model info in the footer:

```
Co-authored-by: Lucy <hugogu.bot@outlook.com>
AI-model: <model-id>
```

Use the model identifier from the session context (e.g., `kimi-coding/k2p5`, `zai/glm-5`).
...
- **Author**: The user who initiated the work (e.g., `Hugo Gu <hugogu@outlook.com>`)
- **Committer**: The AI agent (e.g., `Lucy <hugogu.bot@outlook.com>`)

This is achieved using the `--author` flag.

**New commit:**

```bash
git commit --author="Hugo Gu <hugogu@outlook.com>" -m "$(cat <<'EOF'
...
```bash
git commit --amend --author="Hugo Gu <hugogu@outlook.com>" -m "$(cat <<'EOF'
<updated message>
