...
**Type:** {comp.get('website_type', 'Unknown').replace('_', ' ').title()}  
**Confidence:** {comp.get('confidence', 'N/A')}  
**Purpose:** {comp.get('purpose', 'N/A')}

### Use Cases
"""
        for use_case in comp.get('use_cases', ['N/A']):
            report += f"- {use_case}\n"
        
        report += f"""
### Risk Assessment
**Risk Level:** {comp.get('risk_level', 'Unknown').upper()}

### Applicable Jurisdictions
"""
        for jurisdiction in comp.get('jurisdictions', []):
            report += f"- {jurisdiction}\n"
        
        report += """
### Compliance Requirements
"""
        requirements = comp.get('compliance_requirements', [])
        if requirements:
            for req in requirements:
                report += f"**{req.get('regulation', 'N/A')}** ({req.get('priority', 'medium').upper()})\n"
                report += f"- {req.get('description', 'N/A')}\n"
                report += f"- Jurisdictions: {', '.join(req.get('jurisdictions', []))}\n\n"
        else:
            report += "No specific compliance requirements identified.\n"
