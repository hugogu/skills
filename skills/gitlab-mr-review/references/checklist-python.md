...
- [ ] **分支完备性**: if/elif/else覆盖所有分支，避免隐式遗漏
- [ ] **循环终止**: 确保while循环有明确终止条件，避免死循环
- [ ] **字典安全**: 使用dict.get()或in检查key存在性，避免KeyError
- [ ] **浮点比较**: 避免直接用==比较浮点数，使用math.isclose
- [ ] **时区处理**: 日期时间使用timezone-aware对象，避免naive datetime混用
- [ ] **拷贝语义**: 区分深浅拷贝，避免修改共享对象产生副作用
- [ ] **并发逻辑**: 线程共享数据使用锁或队列，避免竞态条件
- [ ] **资源泄漏**: 确保文件、网络连接、数据库游标正确关闭
